  ...
  require('mongodb').connect(this.generate_mongo_url(), function(err, conn){
    ...
    conn.collection(collection, function(err, coll){
      ...
      switch (type){
        ...
        break;
        case 'application':
          contents += '<div class="app-table"><h4 data-objid="' + entry._id.toString() + '">' + entry._id.toString() + ' <a href="javascript:void(0)" class="btn btn-danger remove-enrty"> 删除 </a></h4><table class="table table-condensed table-hover"><tr><td>姓名</td><td>性别</td><td>年级</td><td>专业</td><td>手机号码</td><td>QQ</td><td>电子邮箱</td><td>分组意向</td></tr><tr><td>'+entry.stu_name+'</td><td>'+entry.stu_gender+'</td><td>'+entry.stu_grade+'</td><td>'+entry.stu_major+'</td><td>'+entry.stu_cell+'</td><td>'+entry.stu_qq+'</td><td>'+entry.stu_email+'</td><td>'+entry.stu_target+'</td></tr><tr><td>个人简介</td><td colspan="7">'+entry.stu_intro+'</td></tr><tr><td>个人经历</td><td colspan="7">'+entry.stu_exp+'</td></tr><tr><td>预期目标</td><td colspan="7">'+entry.stu_expectation+'</td></tr></table></div>';
        break;
      }
      });
        }
